...
const dryFoodSearch = (type = '', query = '') => {
  const url = `${window.location.origin}/fake-api-data/food/dry/${type}.json`;
  log(url);
  ...
  return fetch(url)
    ...
    .then(dryFood => {
      const filtered = dryFood.filter(food => food.includes(query));
      return filtered;
    });
};
...
const testGrainsWithL = grainsWithL => {
  ...
  it('should be the grains with "l"', () => {
    expect(grainsWithL).to.deep.equal(['barley', 'millet', 'bulgar']);
  });
  console.log('');
};
...
const testGrainsWithLe = grainsWithLe => {
  ...
  it('should be the grains with "le"', () => {
    expect(grainsWithLe).to.deep.equal(['barley', 'millet']);
  });
  console.log('');
};
...
const testNutsWithA = nutsWithA => {
  ...
  it('should be the nuts with "a"', () => {
    expect(nutsWithA).to.deep.equal(['pecan', 'brazil', 'acorn', 'pistachio']);
  });
  console.log('');
};
